...
public class ApplicationController {
    ...
    @PostMapping
    public ResponseEntity<ResponseStructure<Application>> saveApplication(@RequestBody Application application ) {
    	Optional<User> user=userRepository.findById(application.getUser().getId());
    	if (user.isPresent()) {
    		applicationRepository.save(application);
    		ResponseStructure<Application> responseStructure=new ResponseStructure<Application>();
    		responseStructure.setData(application);
    		responseStructure.setMessage("Application hase been subbmitted");
    		responseStructure.setStatus(HttpStatus.OK.value());
        	return new ResponseEntity<ResponseStructure<Application>>(responseStructure,HttpStatus.OK);
